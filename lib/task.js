...
exports.build = function (opts) {
  ...
  stream = gulp.src([
    path.resolve(config.themePath, cssFiles + '.scss'), // 要编译的scss文件
    `!${path.resolve(config.themePath, cssFiles + config.backupSuffix + '.scss')}` // 排除备份文件
  ])
  ...
  for (originalFile in indexes) {
    ...
    if (fs.existsSync(originalPath)) {
      ...
      const backupPath = path.resolve(path.parse(originalPath).dir,
              `${path.parse(originalPath).name}${config.backupSuffix}${path.parse(originalPath).ext}`);
